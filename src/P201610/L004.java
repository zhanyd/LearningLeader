...
public class L004 {
        ...
        while(true){
            ...
            while (iter.hasNext()){
                ...
                if((selectionKey.readyOps() & SelectionKey.OP_ACCEPT) == SelectionKey.OP_ACCEPT){
                    ...
                    executorService.execute(new DoRead(selectionKey));

                }
                else if((selectionKey.readyOps() & SelectionKey.OP_WRITE) == SelectionKey.OP_WRITE){
                    executorService.execute(new DoWrite(selectionKey));
                }
    ...
    static class DoRead implements Runnable{
        SelectionKey selectionKey;
        public DoRead(SelectionKey selectionKey) {
    ...
    static class DoWrite implements Runnable{
        SelectionKey selectionKey;
        public DoWrite(SelectionKey selectionKey) {
            this.selectionKey = selectionKey;
        }
